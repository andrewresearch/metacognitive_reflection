...
class PosTagAnalyser {

  private var sentenceTags: Seq[Seq[String]] = Seq()
  private var sentenceWords: Seq[Seq[String]] = Seq()
  private var codedSentences = ArrayBuffer[CodedSentence]()

  def logger = LoggerFactory.getLogger(this.getClass)

  def addSentenceTags(sentenceTags: Seq[Seq[String]]): PosTagAnalyser = {
    this.sentenceTags = sentenceTags
    this
  }

  def addSentenceWords(sentenceWords: Seq[Seq[String]]): PosTagAnalyser = {
    this.sentenceWords = sentenceWords
    this
  }


  def analyse: PosTagAnalyser = {
    //For each sentence number (idx) analyse the sequence of tags
    sentenceTags.zipWithIndex.foreach { case (tags, idx) =>
      ...
      PhraseTag.posPattern.foreach { case (tagList, phraseTags) =>
        ...
        if(phrases.length!=positions.length) logger.error("Phrases and Positions don't match")
      ...
      codedSentences +=  new CodedSentence(idx,sentenceWords.apply(idx).mkString(" "),metaTags.toArray,subTags.toArray,phraseTags.toArray,selfRatio,othersRatio,sentencePhrases.toArray)
    }
    this
  }
